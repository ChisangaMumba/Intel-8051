...
char getDigit(char asciiChar, char pos)  
{
	...
	{
			char i = asciiChar - '0';
			digit = (pos == 2) ? (digits[i] + DECIMAL_POINT) : digits[i]; //ANDing with DECIMAL_POINT Turns off MSB
	}
	else
			if(asciiChar == '-')
			{
					digit = (pos == 2) ? (digits[0] + DECIMAL_POINT) : 0x40;  //0x40 - minus symbol
			}
			else
					if(asciiChar == ' ')
					{
							digit = (pos == 2) ? (digits[0] + DECIMAL_POINT) : 0x00; //0x00 - blank. used for leading zero suppression
					}
